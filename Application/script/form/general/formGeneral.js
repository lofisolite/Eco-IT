...
function validPseudo(input, errorText){
  if(input.value === ""){
    ...
    errorText.textContent = "Le format du pseudo n\'est pas valide. Seulement lettre ou chiffre avec lettre en premier caractères.";
...
function validName(input, errorText){
  if(input.value === ""){
    ...
    errorText.textContent = "Format non valide, seulement des lettres ou tiret. Ex : Jean-Pierre, jeanne."
    return false;
  } else if (input.value.length <= 2 || input.value.length >= 50){
...
function validImage(input, errorText){
  ...
  if(input.value === ""){
    ...
    return false;
        ...
        errorText.textContent = "Votre fichier n'est pas une image en jpeg, jpg ou png.";
        return false;
    } else {
      errorText.textContent= '';
      return true;
...
function validDescription(input, errorText){
  if(input.value === ""){
    ...
    errorText.textContent = "Le format n\'est pas valide. Il y a des caractères spéciaux non autorisés. Seuls ('-.!?,:;\") sont autorisés";
    return false;
  } else if (input.value.length <= 2 || input.value.length > 500){
    errorText.textContent = "La description est trop courte ou trop longue, entre 2 et 500 caractères.";
    return false;
  } else {
    errorText.textContent='';
    return true;
